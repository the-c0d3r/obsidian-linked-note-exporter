...
describe("FileUtils", () => {
	describe("getLinkedPaths", () => {
		it("should extract standard markdown links", () => {
			...
			const result = LinkExtractor.getLinkedPaths(content);
		...
		it("should handle WikiLink anchors with formatting and aliases", () => {
			const content =
				"[[Procedural type of info#**Definition**|Procedural]]";
			const result = LinkExtractor.getLinkedPaths(content);
	...
	describe("extractCanvasLinks", () => {
		it("should extract file nodes", () => {
			...
			const result = LinkExtractor.extractCanvasLinks(content);
		...
		it("should extract links from text nodes", () => {
			const content = JSON.stringify({
				nodes: [
					{
						type: "text",
						text: "Check [[wiki-link]] and [md-link](file.md)",
					},
				],
			});
			const result = LinkExtractor.extractCanvasLinks(content);
		...
		it("should return empty array for invalid json", () => {
			const result = LinkExtractor.extractCanvasLinks("invalid json");
			expect(result).toEqual([]);
		});

		it("should return empty array if no nodes", () => {
			const result = LinkExtractor.extractCanvasLinks("{}");
			expect(result).toEqual([]);
		});
	});
	...
	describe("matchesIgnore", () => {
		it("should match exact tags", () => {
			expect(FilterUtils.matchesIgnore("#private", ["#private"])).toBe(
				true,
			);
			expect(FilterUtils.matchesIgnore("#public", ["#private"])).toBe(
				false,
			);
		});
		...
		it("should match wildcard patterns", () => {
			expect(
				FilterUtils.matchesIgnore("#personal/journal", ["#personal/*"]),
			).toBe(true);
			expect(
				FilterUtils.matchesIgnore("#personal", ["#personal/*"]),
			).toBe(true);
			expect(
...
describe("sanitizeHeaderPath", () => {
	it("should remove markdown headers", () => {
		const result = HeaderHierarchy.sanitizeHeaderPath([
			"# Header 1",
			"## Header 2",
		]);
	...
	it("should trim and collapse spaces", () => {
		const result = HeaderHierarchy.sanitizeHeaderPath(["  Header   1  "]);
...
describe("normalizeTags", () => {
	it("should handle undefined", () => {
		expect(FilterUtils.normalizeTags(undefined)).toEqual([]);
	});

	it("should handle single string", () => {
		expect(FilterUtils.normalizeTags("tag1")).toEqual(["#tag1"]);
		expect(FilterUtils.normalizeTags("#tag1")).toEqual(["#tag1"]);
	});

	it("should handle array of strings", () => {
		expect(FilterUtils.normalizeTags(["tag1", "#tag2"])).toEqual([
			"#tag1",
			"#tag2",
		]);
	});
...
describe("isPathIgnored", () => {
	it("should match ignored folders", () => {
		expect(
			FilterUtils.isPathIgnored("ignored/folder/file.md", [
				"ignored/folder",
			]),
		).toBeTruthy();
	});
	...
	it("should not match partial folder names", () => {
		expect(
			FilterUtils.isPathIgnored("ignored_folder_like/file.md", [
				"ignored_folder",
			]),
		).toBeFalsy();
	});
	...
	it("should return false for non-matching paths", () => {
		expect(
			FilterUtils.isPathIgnored("valid/folder/file.md", ["ignored"]),
		).toBeFalsy();
	});
});
...
describe("extractLinksFromContent", () => {
	it("should extract correct link text and position", () => {
		...
		const result = LinkExtractor.extractLinksFromContent(content);
	...
	it("should ignore external markdown links", () => {
		const content =
			"[Google](https://google.com) and [Custom](ethis://test)";
		const result = LinkExtractor.extractLinksFromContent(content);
	...
	it("should extract standard markdown links", () => {
		const content =
			"Check [Note 1](Note%201.md) and [Note 2](path/to/Note%202.md)";
		const result = LinkExtractor.getLinkedPaths(content);
	...
	it("should ignore external links", () => {
		const content =
			"[External](https://example.com) and [FTP](ftp://server) and [Custom](ethis://test)";
		const result = LinkExtractor.getLinkedPaths(content);
...
describe("getHeaderPathAtPosition", () => {
	it("should return correct path", () => {
		...
		expect(
			HeaderHierarchy.getHeaderPathAtPosition(100, headings, "File"),
	...
	it("should skip H1 if it matches filename", () => {
		...
		expect(
			HeaderHierarchy.getHeaderPathAtPosition(100, headings, "My Note"),
		).toEqual(["Section"]);
	});
});
...
describe("getFileTags", () => {
	...
	it("should return empty array if no cache", () => {
		...
		expect(FilterUtils.getFileTags(new TFile(), mockApp as any)).toEqual(
			[],
		);
	});
	...
	it("should extract frontmatter and inline tags", () => {
		...
		const result = FilterUtils.getFileTags(new TFile(), mockApp as any);
...
describe("shouldExcludeFile", () => {
	...
	it("should exclude by folder", () => {
		...
		const result = FilterUtils.shouldExcludeFile(
			file,
			mockApp as any,
			["ignored/folder"],
			[],
...
describe("getExportPathsForFile", () => {
	it("should return root path if no headers", () => {
		...
		const result = HeaderHierarchy.getExportPathsForFile(
			file,
			headerMap,
			false,
			"Source",
...
describe("buildHeaderHierarchyAsync", () => {
	...
	it("should return empty map if no cache or headings", async () => {
		mockApp.metadataCache.getFileCache.mockReturnValue(null);
		const result = await HeaderHierarchy.buildHeaderHierarchyAsync(
			sourceFile,
			mockApp as any,
			[],
			new Map(),
...
describe("getBacklinks", () => {
	...
	it("should return files that link to the given file", () => {
		...
		const result = LinkExtractor.getBacklinks(targetFile, mockApp as any);
		expect(result).toHaveLength(1);
